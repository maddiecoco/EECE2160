...
class DE1SoCfpga
{
   ...
   if (virtual_base == MAP_FAILED)		// check for errors
   {
    ...
    {
        int ledval = RegisterRead(LEDR_OFFSET);					
if (-1 < ledNum < 10) 
{ 
    if (state == 1) 
{					
...
RegisterWrite(LEDR_OFFSET, ledval);
    }
    ...
    {
        int switchval = RegisterRead(SW_OFFSET);					
        cout << "Switch Value:  " << switchval << endl;
    }
    
    void PushButtonGet()
    {
        int keyval = RegisterRead(KEY_OFFSET);
int a = keyval;
int val;
...
{
int b = RegisterRead(KEY_OFFSET);
a = b;

 if (a == 1)
{
val = RegisterRead(LEDR_OFFSET);
...
int main()
{
        ...
        object1.Write1Led(ledNum, state);
        object1.ReadAllSwitches();
